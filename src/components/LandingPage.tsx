...
function LandingPage({startGameAction, gameStats}: LandingPageProps) {
  ...
  function displayWipInfo() {
    if (process.env.REACT_APP_WIP) {
      return <div className={"WorkInProgress"}>
        ...
        <p>If you are reading this it is because the game is still a work in progress and I am actively fixing bugs and adding functionality. The site may also randomly break as I test changes so don't expect a working game.</p>
  ...
  return (
      <div className={"LandingPage"}>
        <button className={"PlayButton"} tabIndex={0} onClick={startGameAction} autoFocus={true}>PLAY</button>
